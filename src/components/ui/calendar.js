...
function CustomNav() {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("button", { onClick: () => previousMonth && goToMonth(previousMonth), disabled: !previousMonth, className: cn(buttonVariants({ variant: 'outline' }), 'absolute left-2 top-1 h-7 w-7 p-0 opacity-60 hover:opacity-100 z-10'), "aria-label": "Previous month", children: _jsx(ChevronLeftIcon, { className: "h-4 w-4" }) }), _jsx("button", { onClick: () => nextMonth && goToMonth(nextMonth), disabled: !nextMonth, className: cn(buttonVariants({ variant: 'outline' }), 'absolute right-2 top-1 h-7 w-7 p-0 opacity-60 hover:opacity-100 z-10'), "aria-label": "Next month", children: _jsx(ChevronRightIcon, { className: "h-4 w-4" }) })] }));
}
function Calendar({ className, classNames, showOutsideDays = true, ...props }) {
    return (_jsx(DayPicker, { showOutsideDays: showOutsideDays, className: cn('p-3 relative', className), classNames: {
            ...
            weekday: 'w-10 text-center text-muted-foreground text-xs font-normal',
            week: 'flex w-full mt-2',
            day: cn(buttonVariants({ variant: 'ghost' }), 'w-10 h-10 flex items-center justify-center p-0 font-normal'),
            day_selected: 'bg-indigo-500 text-white hover:bg-indigo-500 focus:bg-indigo-500 rounded-md',
            day_today: 'bg-accent text-accent-foreground',
            day_outside: 'opacity-40 invisible',
            day_range_middle: 'aria-selected:bg-indigo-100 aria-selected:text-indigo-800 rounded-none',
